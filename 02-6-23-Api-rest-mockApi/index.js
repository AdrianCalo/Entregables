...
btnActualiza.addEventListener('click', () => {
    ...
    actualizaTabla();//llamo al metodo actualizar tabla
});
//escucho el evento 'ACTUALIZAR' y muestro la tabla modificada
document.addEventListener('actualizar',()=>{actualizaTabla();
});
...
//este metodo se usara con el fin de actualizar la tabla
//cada vez que se haga una modificacion. 
//crearemos un evento que se llamara ACTUALIZA y lo usaremos en cada 
//metodo que modifique algo en la tabla.

function actualizaTabla(){
    fetch(url)
    .then(res=>{
        if(!res.ok){
            throw new Error('error en la solicitud de los datos!');
        }
        return res.json();
    })
    .then(data=> {
        const tabla=document.getElementById('tabla');
        //elimino las filas existentes
        while(tabla.firstChild){
            tabla.removeChild(tabla.firstChild);
        }
        //itero nuevamente sobre los datos obtenidos
        data.forEach(item=> {
            //creamos las celdas y filas nuevamente
            tabla.appendChild(fila);
        }).catch(error=> {
            console.log(error);
            const elementoError= document.getElementById('error');
            elementoError.textContent='se produjo un error al acceder a los datos'
        })
    })
}

//funcion que crea evento ACTUALIZAR

function creaEvento(){
    const actualizarEvent= new Event('actualizar');
    document.dispatchEvent(actualizarEvent);
}
